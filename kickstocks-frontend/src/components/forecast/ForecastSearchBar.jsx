...
export default function ForecastSearchBar({ onForecast, isLoading, initialValue = "" }) {
  ...
  function handleSubmit(e) {
    ...
    onForecast(trimmed.toUpperCase());
  }
      ...
      <div className="flex-1">
        ...
        >
          ...
          <input
            ...
            aria-label="Stock ticker for forecast"
            className={`w-full rounded-sm border bg-base-panel py-3.5 pl-11 pr-4 font-tabular text-base text-ink-primary placeholder:text-ink-muted focus:outline-none focus:ring-1 transition-colors duration-250 ${
              showInvalid
                ? "border-bear/50 focus:ring-bear/60"
                : "border-base-border focus:ring-accent/60 focus:border-accent/40"
            }`}
          />
        </motion.div>
        {showInvalid && (
          <motion.p
            ...
            className="mt-1.5 text-xs text-bear-text"
          >
